...
class SequentialOnlyProvider(FakeProvider):
...
def test_execute_tool_calls_runs_async_tools_concurrently_in_call_order():
    ...
    # Concurrent execution should complete around the longest sleep (0.15s)
    # plus overhead, while staying below the 0.24s sequential total with CI slack.
    assert elapsed < 0.35
...
def test_execute_tool_calls_honors_provider_parallel_capability():
    events: list[str] = []
